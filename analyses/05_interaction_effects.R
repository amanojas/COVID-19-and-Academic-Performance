...
exam_gpa <- exam_level |>
  feols(fml = score ~ post + lowgpa + online + female + r_black + r_asian + r_hispa + r_other +
    parttime + gpamiss | session + instructor, vcov = "hc1")

exam_gpa_did <- exam_level |>
  feols(fml = score ~ post + lowgpa + post * lowgpa + online + female + r_black + r_asian + r_hispa + r_other +
    parttime + gpamiss | session + instructor, vcov = "hc1")
...
exam_online <- exam_level |>
  feols(fml = score ~ post + online + female + r_black + r_asian + r_hispa + r_other + cumgpa +
    parttime + gpamiss | session + instructor, vcov = "hc1")

exam_online_did <- exam_level |>
  feols(fml = score ~ post + online + post * online + female + r_black + r_asian + r_hispa + r_other + cumgpa +
    parttime + gpamiss | session + instructor, vcov = "hc1")
...
results_did |>
  ...
  add_header_above(c(" ", "Final Exam Score" = 2, "Did Student Get The Answer Correct (Y/N)?" = 2),
...
online_models_un <- list(exam_online, exam_online_did, ques_online, ques_online_did)

saveRDS(gpa_models_un ,"S:\\CUNY Coursework\\Paper2\\Aman_E1001\\journal_article\\unstandardised-interaction-models-gpa.rds")
saveRDS(online_models_un ,"S:\\CUNY Coursework\\Paper2\\Aman_E1001\\journal_article\\unstandardised-interaction-models-online.rds")
