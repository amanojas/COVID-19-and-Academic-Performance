...
modelsummary(
  ...
  add_footer_lines(
    values = "* p < 0.1, ** p < 0.05, *** p < 0.01. Final exam scores are based on a 100-point scale. Heteroskedasticity-robust standard errors are used. All regressions include controls for: cumulative GPA, gender, race, age, sophomore status, part-time status, and imputed GPA dummy. Includes instructor and session fixed effects."
  ) |>
...
modelsummary(
  ...
  delete_part(part = "footer") |>
  add_footer_lines(
    values = "* p < 0.1, ** p < 0.05, *** p < 0.01. Heteroskedasticity-robust standard errors are used. Controls included: cumulative GPA, gender, race, part-time status, and imputed GPA dummy. Includes session and instructor fixed effects."
  ) |>
  autofit() |>
  fit_to_width(max_width = 6.5) |>
  fontsize(size = 9, part = "all") |>
